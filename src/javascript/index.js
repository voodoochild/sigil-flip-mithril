;(() => {
  ...
  let Sigils = new Collection(require('./sigils.json'), ['id', 'name']);
  ...
  function getItemDetails () {
    util.batchRequest(`${GW2_API}/items`, ids)
      .then(items => {
        items.forEach(item => {
          let thing = Weapons.item('id', item.id) || Sigils.item('id', item.id);
  ...
  let SigilFlip = {
    controller: function () {
      this.interval = null;
      this.start = () => this.interval = setInterval(getItemPrices, 60000);
